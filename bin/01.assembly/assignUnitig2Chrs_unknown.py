...
for k in sorted(records.keys()):
    ...
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    aln = open(outdir + "/" + k + ".unknown.besthit" , 'w')
    fh = open(outdir + "/" + k + ".unknown.forward.fa", 'w')
    hl = open(outdir + "/" + k + ".unknown.highlight.txt", 'w')
    for i in records[k]:
        aln.write("\t".join(i))
        aln.write('\n')
        unitig, besthit = i
        strand = besthit.split()[-1]
        hl.write(f"{unitig}\t{0}\t{lens[unitig]}\tyellow\n")
        if strand == '-':
